 ...
 */
export function polynomialRoots(f: Polynomial, startInterval: number = -1000, endInterval: number = 1000, epsilon: number = 1e-8): number[] {
    if (f.degree === 2) {
...
function multSign(v: number, sign: number) {
    return v * (sign < 0 ? -1 : 1);
}
